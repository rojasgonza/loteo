...
export default function ListadoProduccionPro() {
    ...
    const fetchProducciones = async () => {
        ...
        setError(null);
        try {
            const res = await authFetch(`/produccion`);
...
<body>
  ...
  ${modalProduccion.fecha_congelado
                ? `<div><strong>Fecha Venc.Congelado:</strong> ${modalProduccion.fecha_congelado.split('T')[0]}
                 <p style="font-size: 70%">(Vida util post descongelado es de 72hs)</p></div>
                                ...
                                {paginatedData.length ? (
                                    paginatedData.map(p => (
                                        <tr key={p.id} style={{ transition: 'background-color 0.3s', cursor: 'default' }} onMouseEnter={e => (e.currentTarget.style.backgroundColor = '#d1fae5')} onMouseLeave={e => (e.currentTarget.style.backgroundColor = 'transparent')}>
                                            ...
                                            <td style={{ padding: '12px 20px', borderBottom: '1px solid #e5e7eb' }}>{p.fechaProduccion.split('T')[0] || "Desconocido"}</td>
                                            <td style={{ padding: '12px 20px', borderBottom: '1px solid #e5e7eb' }}>{p.fecha_vencimiento.split('T')[0] || "Desconocido"}</td>
