...
export default function Insumos() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        // EDITAR
        res = await authFetch(`/insumos/${editingId}`, {
          method: 'PUT',
          headers: {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await authFetch(`/insumos/${id}`, {
        method: 'DELETE'});
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead style={{ backgroundColor: '#d1fae5', position: 'sticky', top: 0 }}>
                  <tr>
                    ...
                    <th style={{ padding: '12px 20px', borderBottom: '1px solid #d1d5db', textAlign: 'left', fontWeight: '600', fontSize: '0.875rem' }} hidden={user?.nivel === 1 || user?.nivel === 2 ? true : false}>Acciones</th>
                  </tr>
                </thead>
                <tbody>
                  {paginatedData.length > 0 ? (
                    paginatedData.map(i => (
                      <tr key={i.id} className="hover:bg-gray-100">
                        ...
                        <td style={{ padding: '12px 20px', borderBottom: '1px solid #e5e7eb' }} hidden={user?.nivel === 1 || user?.nivel === 2 ? true : false} className='py-2 px-4 space-x-2'>
                          <button
                            onClick={() => handleEdit(i)}
                            hidden={user?.nivel === 1 || user?.nivel === 2 ? true : false}
