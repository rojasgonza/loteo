...
export default function LotesInsumos() {
  ...
  const fetchLotes = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_URL}/lotes_insumos`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let res;
      if (editingId) {
        res = await fetch(`${API_URL}/lotes_insumos/${editingId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(body),
        });
      } else {
        res = await fetch(`${API_URL}/lotes_insumos`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(body),
        });
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`${API_URL}/lotes_insumos/${id}`, {
        method: 'DELETE',
      });
      if (!res.ok) {
        await Swal.fire({
...
<body>
  ...
  <div class="producto">cod: ${l.Insumo?.id || 'N/D'} ${l.Insumo?.nombre || 'Insumo Desconocido'}</div>
