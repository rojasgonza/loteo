...
export default function ProductosFinales() {
  ...
  const fetchProductos = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_URL}/productos_finales`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let res;
      if (editingId) {
        res = await fetch(`${API_URL}/productos_finales/${editingId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(formData),
        });
      } else {
        res = await fetch(`${API_URL}/productos_finales`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(formData),
        });
      ...
      if (editingId) {
        setProductos(productos.map(p => (p.id === editingId ? saved : p)));
        setEditingId(null);
      } else {
  ...
  const handleEdit = (producto: ProductoFinal) => {
    setFormData({ nombre: producto.nombre, unidad_medida: producto.unidad_medida });
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`${API_URL}/productos_finales/${id}`, {
        method: 'DELETE',
      });
  ...
  const paginatedData = productos.slice((page - 1) * rowsPerPage, page * rowsPerPage);

  return (
    <main style={{ maxWidth: '80%', margin: "auto", padding: "24px", backgroundColor: "#f9fafb", minHeight: "100vh" }}>
      <h1 className="text-3xl font-bold mb-6 text-center text-blue-700">Productos Finales</h1>

      <form onSubmit={handleSubmit} className="mb-8 bg-white p-6 rounded shadow">
        <h2 className="text-xl font-semibold mb-4">{editingId ? 'Editar Producto' : 'Agregar Producto'}</h2>
                ...
                {paginatedData.length > 0 ? (
                  paginatedData.map(p => (
                    <tr key={p.id} className="hover:bg-gray-100">
                      ...
                      <td style={{ padding: '12px 20px', borderBottom: '1px solid #e5e7eb' }} >{p.unidad_medida}</td>
