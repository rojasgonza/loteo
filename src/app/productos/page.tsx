...
export default function ProductosFinales() {
  ...
  const fetchProductos = async () => {
    ...
    setError(null);
    try {
      const res = await authFetch(`/productos_finales`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let res;
      if (editingId) {
        res = await authFetch(`/productos_finales/${editingId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(formData),
        });
      } else {
        res = await authFetch(`/productos_finales`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(formData),
        });
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await authFetch(`/productos_finales/${id}`, {
        method: 'DELETE'
      });
